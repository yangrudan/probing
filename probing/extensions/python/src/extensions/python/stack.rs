...
pub fn get_python_stacks(pid: py_spy::Pid) -> Result<Vec<CallFrame>> {
    let mut frames = vec![];
    ...
    let mut process = py_spy::PythonSpy::new(pid, &config)
        .with_context(|| format!("Failed to attach Python stack tracer to process {pid}"))?;

    // get stack traces for each thread in the process
    let traces = process
        .get_stack_traces()
        .with_context(|| format!("Failed to get Python stack traces for process {pid}"))?;
    ...
    for trace in traces {
        ...
        let has_module_as_last_frame = trace
            .frames
            .last()
